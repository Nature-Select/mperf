...
- **iOS cold-start timing** (`crates/ios/src/{startup,core_profile_session_raw}.rs`) uses a bespoke DTX channel for `com.apple.instruments.server.services.coreprofilesessiontap` — idevice's `RemoteServerClient` would crash its shared reader on the first kdebug raw-bytes push because it unconditionally NSKeyedArchive-decodes payloads. Each measurement opens a fresh transport, sends `setConfig:` + `start` (py-ios-device's 17-entry `kdf2` filter), waits for the device's bplist-wrapped kdebug events (parses via `extract_inner_data` — the kd_buf records are inside an NSData field of a DTKTraceTapMessage, NOT raw on the wire as pymobiledevice3's docstring claims), and finds the latest class-0x2B event past `mti.mach_absolute_time` as a proxy for "first frame done". Matches PerfDog's ~213ms top-line on iPhone 14 / iOS 26.4.2.
- **iOS kperf is single-owner with ~30s release latency**. After a coreprofile session closes, the in-kernel kperf lock isn't released until the DTServiceHub child fully exits, which on iOS 26.4.2 takes 20-30+ seconds. Within that window, a fresh `setConfig:` returns a `DTTapStatusMessage` with `_lockKPerf: could not lock kperf. Likely another session just started.` — host-side retries with sub-second backoff can't bridge this. `measure_cold_start` does 3 attempts with 500ms/1000ms backoff to mask the very-tail-end races, but user-visible reliability requires ~30s between consecutive cold-start measurements. The UI tooltip in `ScreenTab.tsx` notes this; we don't try to enforce it in code. To validate end-to-end without GUI clicks, use `cargo run --example startup_loop -p mperf-ios` (sleeps `INTER_WAIT_SEC` between iterations, default 3).
- **Don't try a persistent coreprofile session** — it's a tempting design (PerfDog probably does it somehow), but real-device test shows iOS 26 only streams kdebug events for the FIRST app launch after the channel mounts; subsequent launches don't re-trigger event emission. The supervisor/broadcast approach was attempted (commits e0d5445…8281426) and reverted; per-attempt fresh session with retry is the workable model.
- **Don't replace coreprofile with activitytracetap to dodge the kperf 30s lock** — investigated 2026-05-20/21 (commits d4d1a34 + e505079 spike + decoder, kept as reference at `crates/ios/examples/activitytrace_spike.rs`). Activitytrace does avoid the kperf lock and supports 3s back-to-back iterations, but its earliest emitted event from the target PID is the first `com.apple.UIKit/UpdateCycle/UpdateSequence Begin` signpost which fires when UIKit starts its first frame transaction — measured 460-580ms post-mti, consistently ~250ms later than coreprofile's `last class-0x2B kernel event` (213ms, matches PerfDog). The 250ms gap is device-side real, not buffer lag. With `onlySignposts=1` no signposts are emitted during dyld load / app init / first viewDidLoad; the kernel marker fires before user-space framework code has emitted its first signpost. activitytrace measures a strictly later signal — useful for frame-transaction work, but not a substitute for cold-start.
...
- `adb_binary()` reads `MPERF_ADB_PATH`. Tauri setup sets it from bundled sidecar; falls back to system adb.
